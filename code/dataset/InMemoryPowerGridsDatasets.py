...
class UK(InMemoryDataset):
    ...
    # self.processed_paths is created by the dataset class itself (inbuilt), so inorder to change the location, we need to change and give the location itself
    names = {
        "uk": ["Uk", "uk", None, None]
    }
    raw_path = 'ukbig+expmask/'
    def __init__(self, root, name, datatype='Binary', transform=None, pre_transform=None):

        self.datatype = datatype.lower()
        self.name = name.lower()
        self.raw_path = os.path.join(root, 'raw') # added
        assert self.name in self.names.keys()
